      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {["Recipes", "Categories", "30-Minute Meals", "Trending"].map((item) => (
              <li key={item}>
                <Link to="/recipes" className="hover:text-primary transition-colors">{item}</Link>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          <h4 className="font-display text-sm font-semibold text-background mb-4">Stay Connected</h4>
          <p className="text-sm text-background/60 mb-3">Follow us for daily recipe inspiration.</p>
          <div className="flex gap-3">
            {["Instagram", "Pinterest", "YouTube"].map((s) => (
              <span key={s} className="text-xs px-3 py-1.5 rounded-full border border-background/20 hover:border-primary hover:text-primary transition-colors cursor-pointer">
                {s}
              </span>
            ))}
          </div>
