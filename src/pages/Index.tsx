...
import { recipes, categories, getFeaturedRecipes, getTrendingRecipes } from "@/data/recipes";
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-xl animate-fade-in">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <Link to="/recipes">
                <Button size="lg" variant="outline" className="border-background/30 text-background hover:bg-background/10 rounded-full px-8">
                  30-Minute Meals
                </Button>
              </Link>
            </div>
      ...
      <section className="container mx-auto px-4 py-20">
        <div className="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground mt-1">Featured Recipes</h2>
          </div>
          <Link to="/recipes" className="text-sm font-medium text-primary hover:underline hidden sm:block">
            View all →
          </Link>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {getFeaturedRecipes().map((r) => (
      ...
      <section className="bg-secondary/50 py-20">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <span className="text-sm font-medium text-primary uppercase tracking-wider">Browse by</span>
            <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground mt-1">Categories</h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {categories.map((c) => (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">{c.count} recipes</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="container mx-auto px-4 py-20">
        <div className="flex items-end justify-between mb-10">
          <div>
            <span className="text-sm font-medium text-primary uppercase tracking-wider">What's Hot</span>
            <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground mt-1">Trending Now</h2>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {getTrendingRecipes().map((r) => (
            <RecipeCard key={r.id} recipe={r} />
          ))}
        </div>
      </section>
      ...
      <section className="bg-primary/5 py-20">
        <div className="container mx-auto px-4 text-center max-w-lg">
          ...
          <h2 className="font-display text-3xl font-bold text-foreground mb-3">Get Weekly Recipes</h2>
          <p className="text-muted-foreground mb-8">
            Join 10,000+ food lovers. New recipes delivered to your inbox every week.
          </p>
          <form
